...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if (token != null && tokenProvider.validateJwtToken(token)) { //토큰 유효성 검사
            User principal = parseUserSpecification(token); //토큰에서 유저정보 추출
            log.info("JwtAuthenticationFilter -principal : {}", principal);
            AbstractAuthenticationToken authenticated = UsernamePasswordAuthenticationToken.authenticated(principal, null, principal.getAuthorities()); //패스워드 확인 후 권한생성
